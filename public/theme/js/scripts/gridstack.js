...
function loadMainDashboardGrid(){
	...
	var mainWidgets = ["itemSalesSummary", "itemMemberActivity", "itemTasks", "itemFeeds", "itemRecentActivities", "itemBoxes"];
	var serialized_data = getCookie(this.cookieName);
	var items = JSON.parse(serialized_data);
	if (serialized_data == undefined || serialized_data == null || serialized_data == "")
	{
		serialized_data = null;
	}
	if(serialized_data == null){
		items = [{"id":"itemSalesSummary","x":0,"y":10,"width":7,"height":5},{"id":"itemMemberActivity","x":5,"y":0,"width":7,"height":5},{"id":"itemTasks","x":7,"y":10,"width":5,"height":5},{"id":"itemFeeds","x":0,"y":5,"width":6,"height":5},{"id":"itemRecentActivities","x":6,"y":5,"width":6,"height":5},{"id":"itemBoxes","x":0,"y":0,"width":5,"height":5}]
	}
	mainWidgets.forEach(function(widget) {
...
function togglePin(id, toggle){
	...
	var res = _.map($('.grid-stack .grid-stack-item:visible'), function (el) {
    	...
    	if(id == el.attr('data-custom-id')){
        	this.grid.resizable(el, toggle);
        	this.grid.movable(el, toggle);
    	}
	});
}
...
function removeWidget(id){
	...
	var res = _.map($('.grid-stack .grid-stack-item:visible'), function (el) {
    	...
    	if(id == el.attr('data-custom-id')){
    		...
    		elem.style.display = 'none';
        	this.grid.remove_widget(el, false);
    	}
	});
	saveGridLayout();
}
