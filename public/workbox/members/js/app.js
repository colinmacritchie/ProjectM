...
MetronicApp.controller('HeaderController', function($rootScope, $scope, $http, $timeout) {
    $scope.$on('$includeContentLoaded', function() {
        ...
        $scope.loadingBoxesNav();
//        $scope.loadingAlerts();
    });
    
    var pollBoxes = function() {
        $timeout(function() {
            $scope.loadingBoxesNav();
//            $scope.loadingAlerts();
            pollBoxes();
   ...
   $scope.loadingBoxesNav = function(){
	    //Loading Boxes Nav
	...
	//    	    console.log(obj._id);
	    	...
	    	var boxNavData = {id:0,item:boxData};	
	    	var boxNavTree = dhtmlXTreeFromHTML("boxNav_tree"); // for script conversion
	        boxNavTree.enableHighlighting(true);
	        boxNavTree.enableMultiselection(false);
	        boxNavTree.attachEvent("onClick",function(id){
	            var url = String(document.location);
	            if(url.indexOf("/boxes/box") > -1){
		        	document.location.href = boxNavTree.getUserData(id,"href");
	        ...
	        boxNavTree.loadJSONObject(boxNavData); 
	      }, function(err) {
   ...
   $scope.loadingAlerts = function(){
	...
	//    	    newObj.id = i+1;
	//    	    newObj.text = obj.name;
	//    	    newObj.userdata = [{name:"href", content:"#/boxes/box"},{name:"_id",content:obj._id}];
	//    	    boxData[i] = newObj;
	    	}
	//        boxNavTree.loadJSONObject(boxNavData); 
	      }, function(err) {
...
MetronicApp.config(['$stateProvider', '$urlRouterProvider', function($stateProvider, $urlRouterProvider) {
    ...
    $stateProvider
        ...
        .state('dashboard', {
            ...
            data: {pageTitle: 'WorkBox Dashboard'},
                ...
                deps: ['$ocLazyLoad', function($ocLazyLoad) {
                    return $ocLazyLoad.load({ 
                        ...
                        files: [
                            ...
                            'js/controllers/BoxesController.js'  
                        ]                    
                    });
                }]
            }
            ...
            resolve: {
                deps: ['$ocLazyLoad', function($ocLazyLoad) {
                    return $ocLazyLoad.load({
                        ...
                        files: [
                                'js/scripts/box.js',
                                'js/controllers/BoxController.js'  
                        ] 
                    });
                }]
            }
